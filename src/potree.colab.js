...
Potree.WorkerPool = class WorkerPool{
	...
	getWorker(url){
      ...
      // this solution loads the worker's code into a blob
      // then creates the worker: 
      // https://stackoverflow.com/questions/10343913/how-to-create-a-web-worker-from-a-string
      // to load get the code uses an asynchronous get:
		...
		xhr.onload = function (e) {
   		if (xhr.readyState === 4) {
      		if (xhr.status === 200) {
         		...
         		blobworker = xhr.responseText;
         
         		if (this.workers[url].length === 0){
               //	let worker = new Worker(url);
         
         			let blob = new Blob([blobworker], {type: 'application/javascript'});
